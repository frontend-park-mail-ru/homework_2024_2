...
QUnit.module('Тестируем функцию set', function () {
    ...
    QUnit.test('Ошибка обращения к отрицательному элементу массива', assert => {
        assert.throws(() => set({ a: [] }, 'a[-1].b', 16), new RangeError('Addressing an array element with a negative key is invalid'));
    });

    QUnit.test('Ошибка - путь не является строковым типом', assert => {
        assert.throws(() => set({ a: [] }, 456, 16), new TypeError('Path must be a string'));
    });
